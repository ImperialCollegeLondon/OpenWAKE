...
class Ainslie( BaseWake ):
    ...
    def calc_eddy_viscosity( self, F, K1, b, D_m, kappa, ambient_intensity, u_i ):

        #TODO this u_i should be betched from up to date combination model at the proposed location of this turbine
        e_wake = F * K1 * b * D_m * u_i
        e_ambient = F * kappa**2 * ambient_intensity / 10
        ee = e_wake + e_ambient
        return ee

    def calc_filter_function( self, x_rel, turbine_diameter ):
    ...
    def calc_multiplier_grid( self, flow_field ):
        ...
        multiplier_grid = np.ones( ( max_len_x, max_len_r, 2 ) )
        ...
        while i < max_len_x - start_x_index - 1:
            ...
            F = self.calc_filter_function( x, turbine_diameter )
